from numpy import *
"""使用朴素贝叶斯  【文档分类（侮辱言论快速过滤）】    【垃圾邮件分类】
    1.要从文本中获取特征，需要先拆分文本，这里特征是指来自文本的词条token，每个词条是字符的任意组合。词条可以理解为单词，
当然也可以是非单词词条，比如URL、IP地址或者其他任意字符串 
    2.将文本拆分成词条向量后，将每一个文本片段表示为一个词条向量，值为1表示出现#在文档中，值为0表示词条未出现
"""


def load_dataset():
    posting_list = [['my', 'dog', 'has', 'flea', 'problems', 'help', 'please'],     # token(词条)切分后的文档集合
...
def create_vocab_list(dataset):             # 创建一个包含所有文档中出现的不重复词的列表
    ...
    return list(vocabulary_set)             # 返回一个包含所有文档中出现的不重复词的列表,注意list()一下
...
def set_of_words2vec(vocab_list, input_set):                # vocabList:词汇表，inputSet:某个文档
    return_vector = [0] * len(vocab_list)                   # 创建一个所含元素都为0的向量，python自带的方法
...
"""训练算法：丛词向量计算概率
    计算每个类别文档的数目并计算每个类别占总文档数目的比例
    对每一篇文档：
    对每一个类别：
    　　如果词条出现在文档中->增加该词条的计数值                     # 统计每个类别中出现的词条的次数
    　　增加这一类别的所有词条总计数值                               # 统计每个类别的文档中出现的词条总数 
    对每个类别：
...
def train_naivebayes(train_matrix, train_category):
    ...
    for i in range(num_train_documents):            # 遍历训练集trainMatrix中所有文档
        if train_category[i] == 1:                  # 如果侮辱性词汇出现，则侮辱词汇计数加一，且文档的总词数加一
            ...
            pos1_denom += sum(train_matrix[i])      # 数字（注意是加上本行的所有字数）
        else:                                       # 如果非侮辱性词汇出现，则非侮辱词汇计数加一，且文档的总词数加一
            pos0_num += train_matrix[i]             # 数组
            pos0_denom += sum(train_matrix[i])      # 数字
    # pos1_vector = pos1_num / float(pos1_denom)    # 对每个元素做除法求概率,记得要float()
...
def text_parser(big_string):        # 切分文本,接受一个大写字符串并将其解析为字符串列表,bigString：输入字符串
    import re                       # 利用正则表达式来【切分文本】
    list_of_tokens = re.split(r'\W*', big_string)   # 其中分隔符是除单词、数字外的任意字符串。将字符串全部转换成小写.lower()或者大写.upper()
    return [tok.lower() for tok in list_of_tokens if len(tok) > 2]  # 返回切分后的字符串列表,过滤掉长度小于3的字符串。
...
def spam_test():                    # 可以使用词集/词袋模型，此处使用的是词袋函数bag_of_words2vec()
    ...
    for i in range(1, 26):          # 导入文本文件，[1, 25] 正负样本各25个
