...
def randCent(dataSet, k):
    ...
    return centroids                                                # numpy.random.rand(d0, d1, ..., dn) 取值范围[0,1）


"""K-均值聚类算法
创建k个点作为起始质心点（经常是在样本点范围中随机选择）
当任意一个点的簇分配结果发生改变时
    对数据集中的每一个数据点
        对每一个质心
            计算质心与数据点的距离
...
def kMeans(dataSet, k, distMeas=distEclud, createCent=randCent):    # 参数：dataset,num of cluster,distance func,initCen
    ...
    clusterAssment = mat(zeros((m, 2)))        # 存储结果的矩阵，两列分别存储中心点的标号和误距离差
    ...
    while clusterChanged:
        ...
        for i in range(m):                                  # 对每个数据点
            minDist = inf                                   # 无穷大
            minIndex = -1
            for j in range(k):                              # 遍历k个中心点中的每一个，找到距离i点最近的中心点
                distJI = distMeas(centroids[j, :], dataSet[i, :])
                if distJI < minDist:                        # if distance is shorter than minDist
                    minIndex, minDist= j, distJI            # 更新中心点j和距离
...
"""二分K-均值聚类
思想：首先将所有点作为一个簇，然后将该簇一分为二。之后选择能最大程度降低聚类代价函数（也就是误差平方和）的簇划分为两个簇。
...
def biKmeans(dataSet, k, distMeas=distEclud):
    ...
    while (len(centList) < k):                          # 当簇数目小于要划分的k时
        ...
        for i in range(len(centList)):                  # for every centroid(尝试划分每一簇，最后选择划分最大程度降低误差SSE的那一簇）
            ptsInCurrCluster = dataSet[nonzero(clusterAssment[:, 0].A == i)[0], :]  # 得到属于簇i的点
