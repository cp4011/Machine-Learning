...
递归构建决策树:
递归结束的条件：
    1.每个分支下的所有实例都具有相同的类别。    if class_list.count(class_list[0]) == len(class_list): return class_list[0]
    2.程序遍历完所有划分数据集的属性（即所剩余的dataset中只有 label标签列,没有feature特征列了）if len(dataset[0]) == 1：return majority_count(label_list)
  如果所欲实例都具有相同的分类，则得到一个叶子节点或者终止块。任何到达叶子节点的数据必须属于叶子节点的分类
"""
...
def calc_shannon_entropy(dataset):
    ...
    dict = {}
    for line in dataset:                         # 对各类别记数
        label = line[-1]
        dict[label] = dict.get(label, 0) + 1
    entropy = 0.0
    for key in dict.keys():                      # 计算各类别出现的概率并合并进香农熵
        pro = float(dict[key]) / length          # 得到各类别的概率 pro = dict[key] / length
...
def choose_best_feature_to_split(dataset):
    base_entropy = calc_shannon_entropy(dataset)    # 计算原始香农熵
    best_info_gain = 0.0                            # 初始化信息增益和最佳特征
    best_feature = -1
    length = len(dataset[0]) - 1                    # 特征的数量 ***注意要减去1，去掉label那一列***
    for i in range(length):                         # 选出最好的划分数据集的特征
        ...
        for j in feat_list:                                     # 计算每种划分方式的信息熵
            ...
            entropy += pro * calc_shannon_entropy(sub_dataset)  # 这里不是-=
...
def create_tree(dataset, labels):
    ...
    if class_list.count(class_list[0]) == len(class_list):  # 【递归结束条件1】类别完全相同则停止划分
        return class_list[0]
    if len(dataset[0]) == 1:                                # 【递归结束条件2】遍历完所有特征时，返回出现次数最多的类别
...
def store_tree(input_tree, filename):
    ...
    with open(filename, 'wb') as fw:
        pickle.dump(input_tree, fw)              # 此处无return


def load_tree(filename):
    import pickle
    fr = open(filename, 'rb')                    # 默认为只读，'r'可不写，注意加上b
    return pickle.load(fr)                       # 返回树信息,记住return
